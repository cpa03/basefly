...
function generateFAQAnswers(locale: "en" | "zh" | "ja" | "ko"): {
  ...
  const trialDays = SUBSCRIPTION_CONFIG.trialPeriodDays;
  const trialDisplay = getTrialPeriodDisplayString(locale, trialDays);
  ...
  switch (locale) {
    case "zh":
      return {
        ...
        trialPeriod: `我们为专业月度和专业年度计划提供${trialDisplay}的免费试用期。这是一个在承诺付费订阅之前体验所有功能的好方法。`,
      };
    case "ja":
      return {
        freePlanCost:
        ...
        trialPeriod: `プロ月額プランとプロ年間プランの両方に${trialDisplay}の無料トライアルを提供しています。これは、有料サブスクリプションを行う前に全ての機能を体験するのに最適な方法です。`,
      };
    case "ko":
      return {
        freePlanCost:
        ...
        trialPeriod: `저희는 프로 월간 및 프로 연간 플랜에 대해 ${trialDisplay} 물료 체험 기간을 제공합니다. 유료 구독을 하기 전에 모든 기능을 경험할 수 있는 좋은 방법입니다。`,
      };
    default: // en
      return {
        freePlanCost:
        ...
        trialPeriod: `We offer a ${trialDisplay} free trial for both the Pro Monthly and Pro Annual plans. It's a great way to experience all the features before committing to a paid subscription.`,
      };
  }
}
