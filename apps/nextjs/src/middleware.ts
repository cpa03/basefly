...
import { getCSPHeaderValue } from "~/config/csp-config";
 ...
 * - Adds Content-Security-Policy header for XSS protection (configurable via environment variables)
 */
export default async function middleware(req: NextRequest) {
  ...
  if (result && typeof result === 'object' && 'headers' in result) {
    ...
    result.headers.set('Content-Security-Policy', getCSPHeaderValue());
  }
