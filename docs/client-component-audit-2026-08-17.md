...
| `components/video-scroll.tsx`                               | Loaded via `next/dynamic` from a server component (must be client; children are client components) |
...
1. **Close criterion #1** (audit) — this document satisfies it. **All 39 client
   directives are justified.**
2. **Criterion #2 is N/A** — no convertible components exist.
...
4. **Future optimization path** (not part of #723): the remaining client bundle
   weight comes from *framework* code (`@clerk`, `framer-motion`, `@trpc/*`,
