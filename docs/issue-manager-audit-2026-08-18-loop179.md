# Issue Manager Audit Report — 2026-08-18 (Loop 179)

## Executive Summary

- **Open PRs**: 0 at phase entry → **ISSUE MANAGER MODE** engaged (82 open
  issues)
- **Token permissions re-probed** (unchanged from loops 159–178):
  - `issues: write` **NOT available** → issue label normalization, comments,
    ...
    `addComment`, `closeIssue`, `createIssue` — verified via GraphQL)
  - `workflows: write` **NOT available** → pushing changes to
    `.github/workflows/*` is refused at the git protocol level (re-verified
    this loop with a real push attempt)
...
- **Issue tracker is badly out of sync with the codebase**: 68 of 82 open
  issues are **verified resolved in `main`** (65 from loop 178 + 3 newly
  verified this loop: #609, #684, #688); 9 are duplicates of other open
  issues (7 groups); 5 are genuinely unresolved.
- **Repair attempt**: Issue #305 (pnpm consistency in `iterate.yml`) —
  fix written and pushed, but **BLOCKED**: GitHub refuses pushes touching
  `.github/workflows/iterate.yml` without the `workflows` permission.
  Patch documented in loop 178 report; unchanged this loop.
...
GraphQL. **Recommended assignments** (for a maintainer with `issues: write`)
are unchanged from loop 178:
...
9 duplicate groups identified (canonical issue listed first). All duplicates
...
### Newly verified this loop (3 issues)

These were not explicitly covered in loop 178's evidence table; each was
verified against `main` this loop:

| Issue | Evidence (files / commits) |
| ----- | -------------------------- |
| #609 Zod schema consolidation | `packages/api/src/router/schemas.ts` contains `enhancedK8sClusterCreateSchema`, `enhancedK8sClusterDeleteSchema`, `enhancedK8sClusterUpdateSchema`, `enhancedInsertCustomerSchema`, etc. with comment "single source of truth for validation rules (issue #609)"; `router/k8s.ts` and `router/customer.ts` import from `schemas.ts` (no inline duplicates) |
| #684 root build script | root `package.json` has `"build": "pnpm env:validate && turbo build"`; `turbo.json` defines build pipeline with outputs |
| #688 middleware.ts | middleware.ts intentionally **removed** in favor of `apps/nextjs/src/proxy.ts` (PR #981); proxy.ts contains CSP headers, CSRF validation (`validateCSRF`), Clerk auth, security headers — supersedes the issue's request |

### Previously verified (65 issues, loop 178)

Each issue below was verified **resolved in `main`** in loop 178. All are
candidates for closure with a reference to the evidence:
...
**Attempt this loop**: Branch `fix/305-pnpm-iterate-yml-179` created with the
`npm ci || true` → `pnpm install --frozen-lockfile || true` replacement
(2 lines) and pushed.
...
The full patch (pnpm/action-setup, cache paths, cache keys) remains
documented in the loop 178 report and requires a maintainer with
`workflows` permission.

**Validator output** (current, this loop):
...
## Genuinely Unresolved Issues (5)

| Issue | Scope | Why not fixed this loop |
| ----- | ----- | ----------------------- |
| #305 (pnpm in `iterate.yml`) | CI | **BLOCKED** — `workflows` permission required to push `.github/workflows/*`; patch ready (loop 178 report) |
| #650 (extract AI prompts from `on-pull.yml`) | DX | **BLOCKED** — same `workflows` permission restriction; ~360-line embedded prompt needs extraction to a docs file |
...
| 2026-08-18 | Duplicate detection | issues | 9 groups identified (documented above) |
| 2026-08-18 | Resolved verification | #609, #684, #688 | All 3 newly verified with evidence (STEP 3) |
| 2026-08-18 | Repair: push pnpm fix | `fix/305-pnpm-iterate-yml-179` | **BLOCKED** (workflows permission) |
| 2026-08-18 | Baseline health | repo | typecheck 9/9 ✅, lint 9/9 ✅, tests 2165/2165 ✅, CI validator 0 errors ✅ |
| 2026-08-18 | Audit report | `docs/issue-manager-audit-2026-08-18-loop179.md` | written, PR created |
...
- **waiting for human review** — this report requires a maintainer with
  ...
  force-pushed; test artifacts were cleaned up; the blocked patch is fully
  documented in the loop 178 report.
