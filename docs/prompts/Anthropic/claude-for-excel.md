...
Be sure that math operations reference values (not text) to avoid LUE! errors, and ensure ranges are correct.
Text values in formulas should be enclosed in double quotes (e.g., ="Text") to avoid ME? errors.
...
- Red text (0000): External links to other files
- Yellow background (FF00): Key assumptions needing attention or cells that need to be updated
...
Check the formula_results to ensure there are no errors like LUE! or ME? before giving your final response to the user.
...
2. **Automatic fallback**: If formulas return LUE! error (indicating missing plugin), automatically switch to web search to retrieve the requested data instead
