...
/**
 * Alert design tokens
 * Centralized layout, styling, accessibility roles, and micro-interactions for Alert component
 */
export const ALERT_TOKENS = {
  /** Root alert base layout and transitions */
  base: "relative w-full rounded-lg border p-4 transition-all duration-200 [&>svg]:absolute [&>svg]:text-foreground [&>svg]:left-4 [&>svg]:top-4 [&>svg+div]:translate-y-[-3px] [&:has(svg)]:pl-11",
 ...
 * Centralized layout, backdrop, overlay, content, and action button tokens for AlertDialog component
 */
export const ALERT_DIALOG_TOKENS = {
  portal: "fixed inset-0 z-50 flex items-end justify-center sm:items-center",
  overlay: "fixed inset-0 z-50 bg-background/80 backdrop-blur-sm transition-opacity animate-in fade-in",
  content: "fixed z-50 grid w-full max-w-lg scale-100 gap-4 border bg-background p-6 opacity-100 shadow-lg animate-in fade-in-90 slide-in-from-bottom-10 sm:rounded-lg sm:zoom-in-90 sm:slide-in-from-bottom-0 md:w-full",
  ...
  cancel: {
    base: "mt-2 sm:mt-0 transition-all duration-200 ease-out",
    hoverScale: "hover:scale-[1.02]",
    activeScale: "active:scale-[0.98]",
  },
} as const;
